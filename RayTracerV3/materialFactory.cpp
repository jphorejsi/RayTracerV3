...
// Factory method to create a Material with specified properties
std::shared_ptr<IMaterial> MaterialFactory::createMaterial(const Color& od, const Color& os, double ka, double kd, double ks, double n) {
    ...
    return std::make_shared<Material>(od, os, ka, kd, ks, n);
}

// Factory method to create a ReflectiveMaterial with specified properties
std::shared_ptr<IMaterial> MaterialFactory::createReflectiveMaterial(const Color& od, const Color& os, double ka, double kd, double ks, double n, double alpha, double eta) {
    // Check if color values are within the range [0, 1]
    if (od.getR() < 0 || od.getR() > 1 || od.getG() < 0 || od.getG() > 1 || od.getB() < 0 || od.getB() > 1) {
        throw std::invalid_argument("Error: Diffuse color (od) values must be between 0 and 1.");
    }
    ...
    // Check if reflectivity (alpha) is within the range [0, 1]
    if (alpha < 0 || alpha > 1) {
        throw std::invalid_argument("Error: Reflectivity (alpha) must be between 0 and 1.");
    }
    // Check if refractive index (eta) is non-negative
    if (eta < 0) {
        throw std::invalid_argument("Error: Refractive index (eta) must be non-negative.");
    }

    return std::make_shared<ReflectiveMaterial>(od, os, ka, kd, ks, n, alpha, eta);
}
